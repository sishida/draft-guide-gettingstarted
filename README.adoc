...
**LC: General TODO: Tidy up naming (eg in pom.xml and Dockerfile) and make the variables in pom.xml more consistent and simple.**
...
By default, the runnable JAR includes all the features installed with Open Liberty so the package (`target/LibertyProject.jar`) is about 97 MB.

Edit the `<packaging.typ>` line in the `pom.xml` to 'minify' the package size:
...
Run the `mvn clean package` command to create a new `LibertyProject.jar` file.

A minimal server package contains only the features configured in the server. The `server.xml` includes only the features needed by the application and so only those capabilities are included in the package (as well as any they depend on and any required for production deployment). This means the original 97 MB Open Liberty package is now less than 39MB.
...
Go into the `target` directory and run the runnable JAR: `java -jar mpserver.jar`
...
There is a Dockerfile in the `resources` directory. This Dockerfile is based on the Open Liberty docker image from Docker Hub and adds in the project's server configuration and applications from an Open Liberty 'usr server package'. A usr server package contains only an application and server configuration (not a runnable server) and is designed to be unzipped over an existing Open Liberty installation.

To build a usr server package instead of the minimal runnable JAR, edit the `<packaging.type>` line in the `pom.xml` again to be:
...
**LC: Or should I add the 'usr-package' profile into the pom.xml and run the 'mvn -P usr-package install' command as Graham did? Which is best practice?**

Run the packaging command to create the usr server  package: `mvn clean package`

This creates a ZIP file containing just the `usr` directory and its contents `target/LibertyProject.zip` which you can now run in Docker.

If the server is already running, stop it (`mvn liberty:stop-server` or CTRL + C).

In the `resources` directory, build the Docker image from the `Dockerfile`: `docker build -t openliberty:gettingstarted .`


Run the Docker image: `docker run -p 9080:9080 -p 9443:9443 my-demo:gettingstarted`
